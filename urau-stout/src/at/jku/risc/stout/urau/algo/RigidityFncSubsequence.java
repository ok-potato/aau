...
public class RigidityFncSubsequence extends RigidityFnc {
        ...
        for (int i = 0; i < lenL; ) {
            ...
            for (int j = 0, prevLeft = 0, currLen; j < lenR; ) {
                if (tLeft.equals(right.get(j))) {
                    ...
                    currLen = Math.max(prevLeft, prevTop);
                    prevLeft = currLen;
                }
                currLenI[j] = currLen;
            }
    ...
    protected void backTrack(AlignmentList alignList, int branchNum, int[][] traceLen, TermAtomList left, int idxL, int idxT) {
        ...
        for (int i = exitList.size() - 1; i >= 0; i--) {
            ...
            idxL = exit[0];
            idxT = exit[1];
            backTrack(alignList, alignList.size() - 1, traceLen, left, idxL, idxT);

...
public class AntiUnifySystem {
    ...
    public void compute(DebugLevel debugLevel, PrintStream debugOut) throws IllegalAlignmentException {
        while (!problemSet.isEmpty()) {
            ...
            AlignmentList rAlignments = rFnc.compute(hedgeEq.getLeft().getHedge().top(), hedgeEq.getRight().getHedge().top());
            if (rAlignments.isEmpty()) {
                ...
                for (int i = rAlignments.size() - 1; i > 0; i--) {
                    AntiUnifySystem sysBranch = branchSystem(debugLevel, debugOut);
        ...
        List<AntiUnifyProblem> store2 = new ArrayList<>();
        ...
        for (int i = store.size() - 1; i >= 0; i--) {
            ...
            if (len == right1.size()) {
                ...
                for (int j = 0; j < len; j++) {
                    ...
                    store2.add(new AntiUnifyProblem(freshVar, left1.get(j), right1.get(j)));
        ...
        if (changed) {
            ...
            for (int i = store2.size() - 1; i >= 0; i--) {
                ...
                for (int j = i - 1; j >= 0; j--) {
                    ...
                    if (left1.equals(aue2.getLeft()) && right1.equals(aue2.getRight())) {
                        substitute(aue1.generalizationVar, aue2.generalizationVar);
                        store2.remove(i);
                        changed = true;
                        continue l1;
    ...
    private AntiUnifySystem branchSystem(DebugLevel debugLevel, PrintStream out) {
        ...
        if (debugLevel == DebugLevel.PROGRESS) {
            out.println("Problem " + branchId + " spawned child problem " + sysBranch.branchId);
            out.println();
        }
    ...
    private AntiUnifySystem cloneSystem(boolean incCount) {
        AntiUnifySystem clone = new AntiUnifySystem(branchPointer, rFnc, problemSet.clone(), incCount);
        clone.store = new ArrayList<>(store);
        clone.sigma = sigma.clone();
        return clone;
    }
    
    private void rRigidDecompose(AntiUnifyProblem hedgeEq, Alignment alignment) throws IllegalAlignmentException {
        ...
        for (int i = 0, n = alignment.size(); i < n; i++) {
            ...
            composeHedge(substiHedge, leftH, rightH, leftStart, rightStart, leftEnd, rightEnd);
            
            // decompose the match
            // {Zk: sk=qk | 1<=k<=n} U A
            TermNode leftT = leftH.get(leftEnd);
            TermNode rightT = rightH.get(rightEnd);
            if (leftT.getAtom() instanceof Variable && rightT.getAtom() instanceof Variable) {
                throw new IllegalAlignmentException("The same variable is not allowed at both sides of an anti-unification equation");
            }
        ...
        // The last case is Yn
        composeHedge(substiHedge, leftH, rightH, leftStart, rightStart, leftH.size(), rightH.size());
        
        // sigma{X -> Y0,f1(Z1),Y1,...,Yn1,fn(Zn),Yn}
        substitute(hedgeEq.generalizationVar, new TermNode(null, substiHedge));
    }
    
    private void composeHedge(Hedge substiHedge, Hedge leftH, Hedge rightH, int leftStart, int rightStart, int leftEnd, int rightEnd) {
